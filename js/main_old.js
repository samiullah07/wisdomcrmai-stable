...
async function getResponse(prompt) {
    ...
    try {
        ...
        // Hide chat audio if not using Google voice
        $(`.chat_${randomID} .chat-audio`).hide();
...
$(document).ready(function(){
    $.getJSON('/schools.json', function(data) {
        const availableTags = data.map(item => item.text);

        $("#floatingInputSchool").autocomplete({
...
$(document).ready(function() {
    ...
    $('.use-suggestion-short').click(function() {
        var suggestionText = $(this).clone() // Clone the clicked div
            .find('p') // Find the paragraph element
            .text().trim(); // Get the text content of the paragraph
